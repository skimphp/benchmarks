...
/**
 * HTTP "Hello World" benchmark — full framework stack over real HTTP.
 *
 * Hits a running SKIM dev server with curl_multi concurrent requests, measures
 * end-to-end latency (request -> PHP boot -> router -> controller -> send).
 * Targets < 2ms average per request.
 *
 * Usage:
 *   php benchmarks/http_hello.php [url] [total_requests] [concurrent]
 *   env BENCH_JSON=1 php benchmarks/http_hello.php ...
 *
 * Output:
 *   Human-readable summary, then on its own line:
 *     __JSON__ {"name":"http_hello",...}
...
if (empty($latencies)) {
    fwrite(STDERR, "http_hello: FAILED — no successful requests (errors: {$errors})\n");
...
echo "http_hello: avg: " . number_format($avg, 2) . " ms | p95: " . number_format($p95, 2) . " ms | rps: " . number_format($rps, 1) . "\n";
